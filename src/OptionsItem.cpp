	...
	if (change_param == 0) {
		...
		if (C_but->isClick() || R_but->isClick()) {
			if (context.len != 0) {
				...
				if (inc_offset != -3)change_param = 1;
			}
		}
		if (L_but->isClick()) {
		...
		if (T_but->isClick() || T_but->get_inc()) Inc();
		if (B_but->isClick() || B_but->get_inc()) Dec();
		C_but->eraseClicks();
		R_but->eraseClicks();
...
void OptionsItem::render() {
	...
	if(count_string_states)
		...
		if(inc_offset > 0 || inc_offset==-2) tft.printf("\t\t%4ud%s", curr_state, format);
		if (inc_offset == -3) { 
			uint16_t adc_data;
...
void OptionsItem::SetTextContext(const char* str, const char* format, uint8_t c) {
	...
	if(c) parametr_context = (char**)os_malloc(c * sizeof(char*));
...
void OptionsItem::Inc() {
		...
		if (curr_state + inc_offset <= max) { curr_state += inc_offset; }
...
void OptionsItem::Dec() {
		...
		if (curr_state - inc_offset >= min) { curr_state -= inc_offset; }
