...
void writeCommand(uint8_t cmd) {
    while (SPI1CMD & SPIBUSY) { asm volatile("NOP\n"); };
    SPI_DC_LOW();
    write(cmd);
    while (SPI1CMD & SPIBUSY) { asm volatile("NOP\n"); };
    SPI_DC_HIGH();
 ...
 void IRAM_ATTR intr_spi(void) {
   ...
   // uint32_t we = READ_PERI_REG(SPI_SLAVE(HSPI_));
    ...
    case CHAR_COMM_2:  
    {
        ...
        if (first_trans_flag) {     first_trans_flag = 0;
            ...
            if (spi_queue[counter_execute].len > 32) setDataBits(512);
            WRITE_PERI_REG(SPI_SLAVE(HSPI_), SPI_TRANS_DONE_EN); SPI_INTR_ENABLE();
        }
        ...
        if (spi_queue[counter_execute].len > 32) { 
            ...
            if (counter_execute == BUFF_SIZE) { counter_execute = 0; }
            else { counter_execute++; } //   ��������� �������� ��� counter_execute
            *id_command = 0;
            if (spi_queue[counter_execute].id == 0) { SPI_INTR_DISABLE(); spi_intr_flag = 0; }
            SPI1CMD |= SPIBUSY;
        }
    ...
    case FILL_RECT_COMM:      
    {
        if (first_trans_flag) { first_trans_flag = 0; 
            ...
            if (spi_queue[counter_execute].len < 10) {   first_trans_flag = 1;
        ...
        if (spi_queue[counter_execute].len > 32) {
            ...
            if (counter_execute == BUFF_SIZE) { counter_execute = 0; } else { counter_execute++; } //   ��������� �������� ��� counter_execute
    ...
    case BUFF_COMM: 
    {
        ...
        if (first_trans_flag) {
            SPI_INTR_DISABLE();
            writeCommand(ST77XX_CASET);
            write32(spi_queue[counter_execute].ya);  addr_16_data = spi_queue[counter_execute].color.ptr;
            writeCommand(ST77XX_RASET);              pixel_num_buff_tr = 0; first_trans_flag = 0;                                    
            write32(spi_queue[counter_execute].xa);  
            writeCommand(ST77XX_RAMWR);              
...
void ST7735_pixel_to_queue(int16_t x, int16_t y, uint16_t color) {
    ...
    if (counter_add == BUFF_SIZE) { counter_add = 0; }
...
void ST7735_Rect_to_queue(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
    ...
    if (!queue_is_Nempty()) {
        WRITE_PERI_REG(SPI_SLAVE(HSPI_), SPI_TRANS_DONE_EN | SPI_TRANS_DONE); spi_intr_flag = 1;  SPI_INTR_ENABLE();
    }
...
void ST7735_char_to_queue(uint16_t x, uint16_t y, unsigned char c, uint16_t color, uint16_t bcolor, uint8_t size) {
    ...
    if (char_mem == NULL) { Serial.println("can't allocate memory"); return; }
    ...
    *((uint16_t*)char_mem + 4) = color;
    ...
    switch (size) {
        case 1: spi_queue[counter_add].id = CHAR_COMM_1; break;
        case 2: spi_queue[counter_add].id = CHAR_COMM_2; break;
    }
