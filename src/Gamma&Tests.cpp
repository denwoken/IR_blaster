...
*/
    ...
    while (1) {
        for (uint32_t j = 0; j <= 0b11111; j++) {
            ...
            ST7735_Rect_to_queue(160 - 16 * 3, j * 4, 16, 4, j << 6); ST7735_Rect_to_queue(160 - 16 * 3, j * 4 + 2, 16, 4, (j << 6) + 1);
            ST7735_Rect_to_queue(160 - 16 * 2, j * 4, 16, 4, j << 11);
            ST7735_Rect_to_queue(160 - 16, j * 4, 16, 4, (j << 11) | (j << 6) | j);//Gray
        }
        ...
        if (B_but->isClick() || B_but->get_inc()) {
            if (!chosen_reg)
            {
                if (GAMCTRP1[3 + counter] == 63) GAMCTRP1[3 + counter] = 0;
                else  GAMCTRP1[3 + counter]++;
            }
            else
            {
                if (GMCTRN1[3 + counter] == 63) GMCTRN1[3 + counter] = 0;
                else  GMCTRN1[3 + counter]++;
            }
        }
        if (T_but->isClick() || T_but->get_inc()) {
            if (!chosen_reg)
            {
                if (GAMCTRP1[3 + counter] == 0) GAMCTRP1[3 + counter] = 63;
                else GAMCTRP1[3 + counter]--;
            }
            else
            {
                if (GMCTRN1[3 + counter] == 0) GMCTRN1[3 + counter] = 63;
                else  GMCTRN1[3 + counter]--;
            }
        }
        if (C_but->isClick())chosen_reg = !chosen_reg;
        if (C_but->isHolded())break;
